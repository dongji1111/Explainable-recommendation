...
def lossfunction_all(rating_matrix, movie_vectors, user_vectors, flag):
    ...
    for i in range(num_pair):
        ...
        if rating_matrix[u1][i1] > rating_matrix[u2][i2]:
            diff = np.dot(user_vectors[u1, :].T, movie_vectors[i1, :]) - np.dot(user_vectors[u2, :].T, movie_vectors[i2, :])
            diff = -diff
...
def selfgradu(rating_matrix, movie_vectors, user_vector):
...
def selfgradv(rating_matrix, movie_vector,user_vectors):
...
def cf_user(rating_matrix, item_vectors, indices, K):
    ...
    for i in range(num_iter):
        # print(i)
        delta_u = selfgradu(index_matrix, item_vectors, user_vector)
        # print("self",delta_u)
...
def cf_item(rating_matrix, user_vectors, indices, K):
    ...
    # SGD procedure:
    for i in range(num_iter):
        delta_v = selfgradv(rating_matrix,movie_vector,user_vectors)
...
def cal_splitvalue(rating_matrix, movie_vectors, indices_like, indices_dislike, indices_unknown, K):
    ...
    if len(indices_like) > 0:
        # print(indices_like)
        like_vector = cf_user(rating_matrix, movie_vectors, indices_like, K)
    ...
    if len(indices_dislike) > 0:
        ...
        dislike_vector = cf_user(rating_matrix, movie_vectors, indices_dislike, K)
    ...
    if len(indices_unknown) > 0:
        ...
        unknown_vector = cf_user(rating_matrix, movie_vectors, indices_unknown, K)
    ...
    if num_user * num_item != 0:
        for i in range(num_pair):
            ...
            if unknown[u1][i1] > unknown[u2][i2]:
                diff = np.dot(unknown_vector[u1, :].T, movie_vectors[i1, :]) - np.dot(unknown_vector[u2, :].T, movie_vectors[i2, :])
                diff = -diff
                value = value + lmd_BPR * np.log(1 + np.exp(diff))
    print(value)
    return value
...
def cal_splitvalueI(rating_matrix, user_vectors, indices_like, indices_dislike, indices_unknown, K):
    ...
    if len(indices_like) > 0:
        # print(indices_like)
        like_vector = cf_item(rating_matrix, user_vectors, indices_like, K)
        like_vector = np.array([like_vector for i in range(len(indices_like))])
    ...
    if len(indices_dislike) > 0:
        # print(indices_dislike)
        dislike_vector = cf_item(rating_matrix, user_vectors, indices_dislike, K)
        dislike_vector = np.array([dislike_vector for i in range(len(indices_dislike))])
    ...
    if len(indices_unknown) > 0:
        # print(indices_unknown)
        unknown_vector = cf_item(rating_matrix, user_vectors, indices_unknown, K)
        unknown_vector = np.array([unknown_vector for i in range(len(indices_unknown))])
    ...
    if num_user * num_item != 0:
        for i in range(num_pair):
            ...
            u2, i2 = c2 // num_item, c2 % num_item
            if like[u1][i1] > like[u2][i2]:
                diff = np.dot(user_vectors[u1, :].T, like_vector[i1, :]) - np.dot(user_vectors[u2, :].T,like_vector[i2, :])
                diff = -diff
                value = value + lmd_BPR * np.log(1 + np.exp(diff))
    ...
    if num_user * num_item != 0:
        for i in range(num_pair):
            ...
            u2, i2 = c2 // num_item, c2 % num_item
            if dislike[u1][i1] > dislike[u2][i2]:
                diff = np.dot(user_vectors[u1, :].T, dislike_vector[i1, :]) - np.dot(user_vectors[u2, :].T, dislike_vector[i2, :])
                diff = -diff
                value = value + lmd_BPR * np.log(1 + np.exp(diff))
    ...
    if num_user * num_item != 0:
        for i in range(num_pair):
            ...
            u2, i2 = c2 // num_item, c2 % num_item
            if unknown[u1][i1] > unknown[u2][i2]:
                diff = np.dot(user_vectors[u1, :].T, unknown_vector[i1, :]) - np.dot(user_vectors[u2, :].T, unknown_vector[i2, :])
                diff = -diff
                value = value + lmd_BPR * np.log(1 + np.exp(diff))
    print(value)
    return value
